       // 이미지 미리보기
        ...
        if($('#userimg').val() == ''){
          ...
          $('.appform-warn.list01').addClass('act');
          e.preventDefault();
        } 
        if ($('#username').val() == ''){
          $('.appform-warn.list02').text('이름을 입력해주세요.');
          $('.appform-warn.list02').addClass('act');
          e.preventDefault();
        } 
        if ($('#userphone').val() == ''){
          $('.appform-warn.list03').text('연락처를 입력해주세요.');
          $('.appform-warn.list03').addClass('act');
          e.preventDefault();
        } 
        if ($('#userlocation').val() == ''){
          $('.appform-warn.list04').text('주소를 입력해주세요.');
          $('.appform-warn.list04').addClass('act');
          e.preventDefault();
        }  
        if ($('#useremail').val() == ''){
          $('.appform-warn.list05').text('이메일을 입력해주세요.');
          $('.appform-warn.list05').addClass('act');
          e.preventDefault();
        } 
        if($('$userintro').val() == ''){
          $('.appform-warn.list06').text('자기소개서를 입력해주세요.');
          $('.appform-warn.list06').addClass('act');
          e.preventDefault();
        }
      });
      ...
      let saveBtn = document.getElementById('inputsave');
      let loadBtn = document.getElementById('inputload');

      saveBtn.addEventListener('click', inputSave);
      loadBtn.addEventListener('click', inputLoad);
