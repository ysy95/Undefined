...
<?php 
    ...
    $masterSQL = "SESLET * FROM user_data Where user_id = '$masterID' and master = 1";
    $masterChk = mysqli_query($conn, $masterSQL);

    if($masterChk < 0){
      die;
    }

    $user_id = $_GET['id_master_of'];
            ...
            <li>
              <a href='../em.php' title='인재채용'>인재채용</a>
                <ul class='sub'>
                  ...
                  <li><a href='./em_read.php' title='채용 조회' id="em_read">채용 조회</a></li>
      ...
      <article class="appform-wrap">
        ...
        <div class="appform-index-title">
          ...
          while($row = mysqli_fetch_array($userselect)){
            echo "<p class='appform-index-info'>" . $row[3] . "님이 지원하신 채용</p>";
          }
